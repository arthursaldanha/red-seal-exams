...
async function seedCarpenter() {
  ...
  await db.insert(course).values({
    ...
    slug: "carpenter",
    name: "Carpenter",
    description: "Red Seal Carpenter Exam Preparation",
    price: 6700,
    currency: "CAD",
    isActive: true,
  });
  ...
  const carpenterDir = join(__dirname, "carpenter");
  const files = readdirSync(carpenterDir)
  ...
  for (const file of files) {
    const data: BlockFile = JSON.parse(
      readFileSync(join(carpenterDir, file), "utf-8")
    );
    ...
    console.log(`  ✓ Block ${data.block}: ${data.blockTitle}`);
  ...
  console.log("\n✅ Seed Carpenter concluído!");
  await pool.end();
}

seedCarpenter().catch((err) => {
  console.error(err);
  pool.end();
  process.exit(1);
});
