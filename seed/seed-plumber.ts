...
async function seedPlumber() {
  ...
  await db.insert(course).values({
    ...
    slug: "plumber",
    name: "Plumber",
    description: "Red Seal Plumber Exam Preparation",
    price: 6700,
    currency: "CAD",
    isActive: true,
  });
  ...
  const courseDir = join(__dirname, "plumber");
  ...
  console.log("\n✅ Seed Plumber concluído!");
  await pool.end();
}

seedPlumber().catch((err) => {
  console.error(err);
  pool.end();
  process.exit(1);
});
