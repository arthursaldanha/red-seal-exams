...
async function seedConstructionElectrician() {
  ...
  await db.insert(course).values({
    ...
    description: "Red Seal Construction Electrician Exam Preparation",
    price: 6700,
    currency: "CAD",
    isActive: true,
  });
  ...
  const courseDir = join(__dirname, "construction-electrician");
  ...
  console.log("\n✅ Seed Construction Electrician concluído!");
  await pool.end();
}

seedConstructionElectrician().catch((err) => {
  console.error(err);
  pool.end();
  process.exit(1);
});
